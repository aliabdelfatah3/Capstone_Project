...
export const MovieCasts = () => {
  ...
  const fetchCasts = () => fetchMovieCasts(id);
  const { data: movieCasts, loading, error } = useFetch(fetchCasts, [id]);
  ...
  if (!movieCasts?.cast?.length) return null;

  return (
    <div className="w-full mt-10 mb-20 px-4 md:px-0 group/swiper" key={movieCasts.id}>
      ...
      <div className="relative">
        ...
        >
          {movieCasts.cast.slice(0, 15).map((cast) => (
